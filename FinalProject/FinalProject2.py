...
def main():
    ...
    try:
        ...
        # Increase timeout
        remote_device.set_sync_ops_timeout(10)
        ...
        def read_task():
            while not stop:
                ...
                # Get RSSI of devices
                RSSI_raw = local_device.get_parameter("DB")
                RSSI = (-1 * int.from_bytes(RSSI_raw, "big"))
                ...
                if (socket.gethostname() == "LAPTOP-T3G01EJ2"):
                    requestLink = "http://api.thingspeak.com/update?api_key=8O0PFDFMUYX0ARAN&field1=" + str(roundedTemp) + "&field2=" + str(roundedPres) + "&field7=" + str(RSSI) #Matthew's PC
                elif (socket.gethostname() == "LAPTOP-F95QGE94"):
                    requestLink = "http://api.thingspeak.com/update?api_key=8O0PFDFMUYX0ARAN&field3=" + str(roundedTemp) + "&field4=" + str(roundedPres) + "&field7=" + str(RSSI) #Tom's PC
                elif (socket.gethostname() == "LAPTOP-F95QGE94"):
                    requestLink = "http://api.thingspeak.com/update?api_key=8O0PFDFMUYX0ARAN&field5=" + str(roundedTemp) + "&field6=" + str(roundedPres) + "&field7=" + str(RSSI) #Ashlee's PC
                else: 
                    print("Host not recognized!")
                    requestLink = "http://api.thingspeak.com/update?api_key=8O0PFDFMUYX0ARAN&field7=" + str(RSSI)
                    #Host not recognized
